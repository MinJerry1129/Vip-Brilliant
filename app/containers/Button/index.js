...
import {
	...
	COLOR_ORANGE, COLOR_WHITE, NAV_BAR_START, NAV_BAR_END,
	...
	render() {
		...
		if(type === 'gradient'){
			return (<RectButton onPress={onPress} style={[styles.gradientButtonContainer, style]} enabled={!(disabled || loading)}>
				<LinearGradient colors={[NAV_BAR_START, NAV_BAR_END]} style={[styles.gradientButtonText, size ? styles[`button_size_${ size }`] : {}]}>
					{
						loading
				...
				>
					...
					}
					{
						loading
							...
							:<>
								{iconCenter?iconCenter(): null}
							<Text style={{ ...(styles.text), color: disabled?themes[theme].auxiliaryText:'black' , fontWeight: (fontWeight ? styles[`font_weight_${ fontWeight }`] : 'normal')}}>{ text || title }</Text>
							</>
					}
				</View>
			</RectButton>
